...
class DbManager {
    ...
    public static function getProjects(){
        $conn = self::connectToDb();
        $sql = $conn->prepare("SELECT * from `projects` WHERE `isDeleted` = 0 AND `published` = 1 ORDER BY itemOrder + 0 ASC");
        $sql->execute();
        $result = $sql->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    ...
    public static  function getManageProject($projectId){
        $conn = self::connectToDb();
        $sql = $conn->prepare("SELECT * from `projects` WHERE `isDeleted` = 0 ORDER BY itemOrder + 0 ASC");
        $sql->execute();
        $result = $sql->fetchAll(PDO::FETCH_ASSOC);
        return $result;
